...
def merge_text_into_one_data_set(name_one,name_two,out_file):
    ...
    return None
def main():
    most_freq_words=["bir","yalnızlık","ve","ne","bu","gibi","ben","de","yalnız","o","sen","her","da","beni","yine","seni"]	
    remove_chars_and_html_from_text("yalnizlik.txt","yalnizlik_clean.txt",11000)
    remove_chars_and_html_from_text("ozlem.txt","ozlem_clean.txt",13000)
    merge_text_into_one_data_set("yalnizlik_clean.txt","ozlem_clean.txt","data.txt")
    Markov_Chain=create_chain("data.txt",)
    for a in range (10):
        create_text(Markov_Chain,"ben",4,5,most_freq_words)
